...
Today, online shopping is manual, fragmented, and exhausting. A buyer must visit dozens of websites, filter through bloated catalogs, manually compare shipping times and prices, copy discount codes, and complete OTP-based checkouts for every purchase.
...
1. **A2A (Agent-to-Agent Protocol)**: The communication bus that enables agents to discover each other via standardized `AgentCard`s, query catalogs, and conduct commercial negotiations without human intervention.
...
3. **AP2 (Agent Payments Protocol)**: The cryptographic security layer. It produces ES256-signed **Cart Mandates** and **Payment Mandates** that prove the user authorized the transaction and the merchant committed to the exact items and price.
...
3. **Proposal & Counter-Offers**: Merchants return structured quotes. If a merchant allows negotiation, the buyer agent initiates automated counter-offers to beat competitor prices within merchant policy limits.
...
5. **Deterministic Policy Gate**: **Before any money moves**, a non-negotiable Python policy engine deterministically checks that the price is within budget, the merchant is whitelisted, and the item matches user requirements.
...
- **The Policy Engine is a deterministic firewall**: It is 100% pure, deterministic code with zero LLM involvement. If an item costs Rs. 5,001 on a Rs. 5,000 budget, the Policy Engine immediately rejects it with `POLICY_REJECTED`.
